...
@section('title', 'My Task Board - Create   | devChallenges.io')
...
@section('content')
            ...
            @foreach ($tasks as $task)
                @switch($task->status)
                    ...
                    @case('Completed')
                        ...
                        @case('ToDo')
                            <a href="{{ route('task.edit', $task) }}" class="in-progress flex w-full bg-[#E3E8EF] gap-5 rounded-lg p-3 justify-between mb-5">
                ...
                <div class="task-details p-4">
                    <div class="flex justify-between items-center mb-4">
                        <h3>Task details</h3>
                        <a href="{{ route('board.show', $task->board_id) }}"><img class="close-details w-8 h-8 p-2 rounded-lg border border-gray-300 cursor-pointer" src="{{ asset('assets/images/close_ring_duotone-1.svg') }}"></a>
                    </div>
                    ...
                    <form action="{{route('task.update', $task) }}" method="POST">
                        ...
                        <div class="flex flex-col mb-4">
                            ...
                            <input class="border border-gray-300 p-2 text-black bg-white rounded" required type="text" name="name" value="{{ old('name', $task->name) }}">
                        </div>

                        <div class="flex flex-col mb-4">
                            <label>Description</label>
                            <textarea class="border border-gray-300 p-2 bg-white rounded resize-none h-40" name="description" placeholder="Enter a short description">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <label class="labelStatus flex justify-between items-center border border-gray-300 border-2 rounded-lg py-0.5 pl-0.5">
                                    <input type="radio" name="status" class="status hidden" value="Progess" @checked(isset($task) && $task->status == "Progress")>
                                ...
                                <label class="labelStatus items-center border border-gray-300 border-2 rounded-lg py-0.5 pl-0.5">
                                    <input type="radio" name="status" class="status hidden" value="Completed" @checked(isset($task) && $task->status == "Completed")>
                                    <div class="flex items-center h-full gap-5">
                                ...
                                <label class="labelStatus items-center border border-gray-300 border-2 rounded-lg py-0.5 pl-0.5">
                                    <input type="radio" name="status" class="status hidden" value="Incompleted" @checked(isset($task) && $task->status == "Incompleted")>
                                    <div class="flex items-center h-full gap-5">
                                ...
                                <label class="labelStatus items-center border border-gray-300 border-2 rounded-lg py-0.5 pl-0.5">
                                    <input type="radio" name="status" class="status hidden" value="ToDo" {{ (isset($task) && $task->status == "ToDo") ? "checked" : '' }}>
                                    <div class="flex items-center h-full gap-5">
                        ...
                        <div w-full class="flex justify-end absolute bottom-2 right-5 gap-3">
                            <form method="POST" action="{{ route('task.destroy', $task) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="flex gap-2 items-center w-fit h-10 px-3 bg-gray-500 rounded-2xl">
                                    <img class="w-5 h-5" src="{{ asset('assets/images/Trash.svg') }}">
                                    <p class="text-white">Delete</p>
                                </button>
                            </form>
                            <button type="submit" class="w-20 h-10 px-3 bg-blue-700 rounded-2xl text-white" >Modify</button>
                        </div>
                    </form>
                </div>
            @endforeach
    ...
    <div class="task-details p-4">
        <div class="flex justify-between items-center mb-4">
            <h3>Task details</h3>
            <a href="{{ route('board.show', $board_id) }}"><img class="close-details w-8 h-8 p-2 rounded-lg border border-gray-300 cursor-pointer" src="{{ asset('assets/images/close_ring_duotone-1.svg') }}"></a>
        </div>

        <form action="{{ route('task.store', $board_id) }}" method="POST">
            @csrf
            <div class="flex flex-col mb-4">
                <label>Task name</label>
                <input class="border border-gray-300 p-2 text-black bg-white rounded" required type="text" name="name">
            </div>
            ...
            <div w-full class="absolute bottom-2 right-5 gap-3">

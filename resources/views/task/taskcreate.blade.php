...
@section('content')
    ...
    <div class="task-details p-4">
        <div class="flex justify-between items-center mb-4">
            <h3>Task details</h3>
            <a href="{{ route('task.index') }}"><img class="close-details w-8 h-8 p-2 rounded-lg border border-gray-300 cursor-pointer" src="{{ asset('assets/images/close_ring_duotone-1.svg') }}"></a>
        </div>

        <form action="{{ (isset($taskSelect)) ? route('task.update', $taskSelect) : route('task.store') }}" method="POST">
            @csrf
            ...
            <div class="flex flex-col mb-4">
                ...
                <input class="border border-gray-300 p-2 text-black bg-white rounded" required type="text" name="name" value="{{ (isset($taskSelect)) ? old('name', $taskSelect->name) : old('name') }}">
            </div>
            ...
            <div class="flex flex-col mb-4">
                ...
                <textarea class="border border-gray-300 p-2 bg-white rounded resize-none h-40" name="description" placeholder="Enter a short description">
                    {{ (isset($taskSelect)) ? old('description', $taskSelect->description) : old('description') }}
                </textarea>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <label class="labelStatus flex justify-between items-center border border-gray-300 border-2 rounded-lg py-0.5 pl-0.5">
                        <input type="radio" name="status" class="status hidden" value="Progess" @checked(isset($taskSelect) && $taskSelect->status == "Progress")>
                    ...
                    <label class="labelStatus items-center border border-gray-300 border-2 rounded-lg py-0.5 pl-0.5">
                        <input type="radio" name="status" class="status hidden" value="Completed" @checked(isset($taskSelect) && $taskSelect->status == "Completed")>
                        <div class="flex items-center h-full gap-5">
                    ...
                    <label class="labelStatus items-center border border-gray-300 border-2 rounded-lg py-0.5 pl-0.5">
                        <input type="radio" name="status" class="status hidden" value="Incompleted" @checked(isset($taskSelect) && $taskSelect->status == "Incompleted")>
                        <div class="flex items-center h-full gap-5">
                    ...
                    <label class="labelStatus items-center border border-gray-300 border-2 rounded-lg py-0.5 pl-0.5">
                        <input type="radio" name="status" class="status hidden" value="ToDo" {{ (isset($taskSelect) && $taskSelect->status == "ToDo") ? "checked" : '' }}>
                        <div class="flex items-center h-full gap-5">
            ...
            <div w-full class="flex justify-end absolute bottom-2 right-5 gap-3">
                <form method="POST" action="{{ route('task.destroy', $taskSelect) }}">
                    @csrf
                    @method('DELETE')
